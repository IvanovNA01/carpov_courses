...
def get_select_vars(mertics, metrics_list, group_by_slices_list, slice, all_data=False):
    ...
    for metric in mertics:
        ...
        expression = '{formula} AS {alias}'.format(
...
def get_alert(alert_data, time, chat):
    ...
    select_vars = get_select_vars(
        mertics, metrics_list, group_by_slices_list, slice)
    from_table_name = get_from_table_name(mertics, metrics_list)
    where_expression = get_where_expression(
        where_expression_template, group_by_slices_list, time, slice)
    ...
    if slice != 'total':
        ...
        for group_level in group_levels:
            ...
            data_to_alerts[group_level] = {
                'df': alert_df, 'slice': slice, 'group_level': group_level}

    else:
        data_to_alerts['total'] = {'df': all_data,
                                   'slice': 'total', 'group_level': 'total'}
    ...
    for group in data_to_alerts:
        ...
        for metric in mertics:
            ...
            alert_data = get_metric_alert(
                alert_df, metric, metric_alias, slice, group, day_threshhold=0.5)
            ...
            select_vars = get_select_vars(
                mertics, metrics_list, group_by_slices_list, slice, True)
            from_table_name = get_from_table_name(mertics, metrics_list)
            where_expression = get_where_expression(
                where_expression_template, group_by_slices_list, time, slice, True)
            group_by_expression = get_group_by_expression(
                slice, group_by_slices_list, True)
            ...
            if slice != 'total':
                plot_df = plot_df.query(
                    "{slice} == '{group}'".format(group=group, slice=slice))

            plot_df['time'] = pd.to_datetime(plot_df['time'])
            plot_df = plot_df[plot_df['time'] <= time]
            alert_plot = alert_plots[slice][metric]

            send_alert(plot_df, group, metric_name, metric_alias,
                       slice_name, alert_plot, responsible_users, chat=chat_id)
